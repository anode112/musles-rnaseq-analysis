...
Lets check values' distribution. Посмотрим распределние матрицы. Очистим от low образцов. Попробуем разделить денсити по образцам. We observe matrix in base state, not in log state.
...
ggplot(expr_vals, aes(x = Expression)) +
  ...
  labs(title = "Density Plot of RNA-seq muscle Expression", x = "TPM expression Level", y = "Density") +
  theme_minimal() + scale_x_continuous(limits = c(1, 20))
